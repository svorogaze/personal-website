...
func createBlog(c *gin.Context) {
	...
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Couldn't save the blog text"})
		log.Print(err)
		return
	}
...
func createBucket() error {
	...
	b, err := minioClient.BucketExists(ctx, "blog-cover-images")
	if err != nil {
		return err
	}
	if !b {
		err := minioClient.MakeBucket(ctx, "blog-cover-images", minio.MakeBucketOptions{})
		return err
	}
	return nil
}
